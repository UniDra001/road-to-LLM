...
基于pytorch框架编写模型训练
这是一个5分类任务
规律：x是一个10维向量，两个为一组，当第i组中两个数的和最大时，输出为第i类，i=0~5
...
class TorchModel(nn.Module):
    def __init__(self, input_size, ):
        ...
        self.loss = nn.BCEWithLogitsLoss()  # 二进制交叉熵损失适用于多分类
...
def build_dataset(total_sample_num):
    X = torch.randn(total_sample_num, 10)  # 100个样本，每个样本有10个特征
    # 计算每两个数之和，组成新的张量
    sums = X[:, ::2] + X[:, 1::2]
    # 找到每行最大值的索引，即最大和的组别
    _, indices = sums.max(dim=1)
    # 生成 one-hot 标签
    Y = torch.eye(5)[indices]
    return X, Y
...
def evaluate(model):
    ...
    accuracy = (predictions == y).sum().item() / y.numel()
