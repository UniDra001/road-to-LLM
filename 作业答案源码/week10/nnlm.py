...
def build_sample(tokenizer, window_size, corpus):
    ...
    x = tokenizer.encode(window, padding='max_length', add_special_tokens=False, truncation=True, max_length=10)
    y = tokenizer.encode(target, padding='max_length', add_special_tokens=False, truncation=True, max_length=10)
    return x, y
...
def generate_sentence(openings, model, tokenizer, window_size):
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            openings += pred_char
            x = tokenizer.encode(openings, padding='max_length', add_special_tokens=False, truncation=True, max_length=10)
